...
public class PathFlowModel {
    ...
    public void initializeParameters() throws FileNotFoundException, GRBException {
        this.lambda = new GRBVar[data.numPeriods][data.numVehicles][data.numTrips][];
        ...
        this.tS = new GRBVar[data.numPeriods][data.numVehicles][data.numTrips];
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            for (int v = 0; v < data.numVehicles; v++) {
                for (int r = 0; r < data.numTrips; r++) {
                    lambda[d][v][r] = new GRBVar[data.pathMap.get(d).get(data.vehicles[v].vehicleType.type).size()]; //creating a jagged array
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            for (int v = 0; v < data.numVehicles; v++) {
                for (int r = 0; r < data.numTrips; r++) {
                    ...
                    tS[d][v][r] = model.addVar(0, data.latestInTime, 0, GRB.CONTINUOUS, variable_name);
                }
            }
        }
    ...
    public void constraint39() throws GRBException {
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            for (int v = 0; v < data.numVehicles; v++) {
                for (int r = 0; r < data.numTrips ; r++) {  // num trips cannot be
    ...
    public void constraint40() throws GRBException {
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            for (int v = 0; v < data.numVehicles; v++) {
                for (int r = 0; r < data.numTrips - 1; r++) {  // num trips cannot be
                    GRBLinExpr lhs = new GRBLinExpr();  //Create the left hand side of the equation
                    lhs.addTerm(1.0, tS[d][v][r]);
                    lhs.addTerm(-1.0, tS[d][v][(r + 1)]);
                    for (Path p : data.pathMap.get(d).get(data.vehicles[v].vehicleType.type)) {
                        lhs.addTerm(p.duration, lambda[d][v][r][p.pathId]);
                        lhs.addTerm(data.latestInTime + data.loadingTime[data.vehicles[v].vehicleType.type], lambda[d][v][r+1][p.pathId]);
                    }
                    ...
                    model.addConstr(lhs, GRB.LESS_EQUAL, data.latestInTime, constraint_name);
                }
            }
        }
    }
    ...
    public void constraint42() throws GRBException {
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            for (int i = 0; i < data.numCustomers; i++) {
                ...
                if (data.possibleDeliveryDays[d][i] == 0)  // TODO: Check if this is correct implemented
                    continue;
                GRBLinExpr lhs = new GRBLinExpr();  //Create the left hand side of the equation
                for (int v = 0; v < data.numVehicles; v++) {
                    for (int r = 0; r < data.numTrips; r++) {
                ...
                String constraint_name = String.format("5.43 -Legal delivery day %d for customer %d: %d (yes:1, no:0)", d, i,data.possibleDeliveryDays[d][i]);
                ...
                for (int r = 0; r < data.numTrips; r++) {
                    for (int i = 0; i < data.numCustomers; i++) {
                        ...
                        for (Path p : data.pathMap.get(d).get(data.vehicles[v].vehicleType.type)) {
                            for (Customer c : p.customers) {
                                if (c.customerID == i) { // TODO: 19.11.2019 can be done quicker?
                                    lhs.addTerm(-data.vehicleCapacity[v], lambda[d][v][r][p.pathId]);
                                }
                            }
                        }
                        String constraint_name = String.format("5.45 -Connection q and lambda for customer %d vehicle %d trip %d day %d. M = %f", i, v, r, d, data.vehicleCapacity[v]);
                        model.addConstr(lhs, GRB.LESS_EQUAL, 0, constraint_name);
                    }
                }
            }
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            for (int v = 0; v < data.numVehicles; v++) {
                for (int r = 0; r < data.numTrips; r++) {
                    ...
                    String constraint_name = String.format("5.46 -Capacity vehicle %d trip %d period %d. Capacity %f", v, r, d, data.vehicleCapacity[v] );
    ...
    public void constraint46() throws GRBException {
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            ...
            // Create name
            String constraint_name = String.format("5.43 -Overtime on day %d. OvertimeLimit %f ", d, data.overtimeLimit[d]);
            ...
            for (int i = 0; i < data.numCustomers; i++) {
                for (int m = 0; m < data.numProducts; m++) {
                    if (data.productTypes[i][m] == 0 && data.productQuantity[i][m] > 0) {
                        ...
                        String constraint_name = String.format("5.44 -Fixed quantity for store %d of product %d on day %d. Fixed quantitiy %f. Number of products: %d", i, m, d, data.productQuantity[i][m], data.numProducts);
            ...
            for (int i = 0; i < data.numCustomers; i++) {
                for (int m = 0; m < data.numProducts; m++) {
                    if (data.productTypes[i][m] == 1  && data.productQuantity[i][m] > 0) {
                        ...
                        String constraint_name = String.format("5.45 a) -Min delivery of dividable product %d customer %d on day %d. Min amount: %f", m, i, d, data.minAmountDivProduct[i][m]);
                        // Activate the constraint
                        model.addConstr(lhs, GRB.LESS_EQUAL, 0, constraint_name);
                    }
                }
            ...
            for (int i = 0; i < data.numCustomers; i++) {
                for (int m = 0; m < data.numProducts; m++) {
                    if (data.productTypes[i][m] == 1  && data.productQuantity[i][m] > 0) {
                        ...
                        String constraint_name = String.format("5.45 b) -Max delivery of div.product %d customer %d on day %d. Max amount %f", m, i, d, data.maxAmountDivProduct[i][m]);
                        // Activate the constraint
                        model.addConstr(lhs, GRB.LESS_EQUAL, 0, constraint_name);
                    }
                }
        ...
        for (int i = 0; i < data.numCustomers; i++) {
            for (int m = 0; m < data.numProducts; m++) {
                if (data.productQuantity[i][m] > 0) {
                    ...
                    String constraint_name = String.format("5.46 -Delivery of product %d to customer %d. Quantity %f", m, i,data.productQuantity[i][m] );
                    // Activate the constraint
    ...
    public void symmetryTrip() throws GRBException {
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            for (int v = 0; v < data.numVehicles-1; v++) {
                ...
                String constraint_name = String.format("5.63 Sym1 - Number of trips used for vehicle %d must be larger than vehicle %d in period %d and vehicle type %d", v, v+1 ,d ,data.vehicles[v+1].vehicleType.type);
                model.addConstr(lhs, GRB.GREATER_EQUAL, 0, constraint_name);
            }
        }
    }
    ...
    public void storePath() throws GRBException {
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            ...
            for (int v = 0; v < data.numVehicles; v++) {
                ...
                for (int r = 0; r < data.numTrips; r++) {
                    for (Path p : data.pathMap.get(d).get(data.vehicles[v].vehicleType.type)) {  // TODO: 23.11.2019 Problem when less than 4 types
                        if (lambda[d][v][r][p.pathId].get(GRB.DoubleAttr.X) == 1) {
    ...
    public void calculateResultValues() throws GRBException {
        ...
        for (int d = 0; d < data.numPeriods; d++) {
            for (int v = 0; v < data.numVehicleTypes; v++) {
                for (Path p : data.pathMap.get(d).get(v)) {
                    numGeneratedTrips++;
                }
            }
        }
